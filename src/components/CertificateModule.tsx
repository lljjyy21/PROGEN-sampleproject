    ...
    <div className="bg-white mx-4 mt-4 p-5 rounded-2xl shadow-sm border border-black/5">
      ...
      <div className="grid grid-cols-2 gap-3">
        {certificates.map((cert, index) => (
          <div 
            key={index} 
            className="group relative aspect-[4/3] rounded-xl overflow-hidden bg-[#EEEFEA] border border-black/5 cursor-pointer"
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
              <p className="text-white text-[10px] font-bold truncate">{cert.title}</p>
            </div>
          </div>
        ))}
      </div>
